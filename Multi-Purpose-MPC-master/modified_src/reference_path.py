...
class ReferencePath:
    ...
    def _construct_waypoints(self, waypoint_coordinates):
        ...
        for wp_id in range(len(waypoint_coordinates) - 1):
            ...
            if wp_id == 0:
                ...
                angle_dif = np.mod(psi - angle_behind + math.pi,
                                   2 * math.pi) - math.pi  # steering angle
    ...
    def update_path_constraints(self, wp_id, N, min_width, safety_margin):
        ...
        for n in range(N):
            ...
            # Append results
            ub_hor.append(ub_ls[0])
            ub_hor.append(ub_ls[1])
            ub_hor.append(np.inf)
            lb_hor.append(lb_ls[0])
            lb_hor.append(lb_ls[1])
            lb_hor.append(-np.inf)
